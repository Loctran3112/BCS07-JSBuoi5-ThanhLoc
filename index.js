...
function diemTongBaMon() {
    ...
    var doiTuong = document.querySelector(".lopDoiTuong input[name='selector']:checked").value;
...
function thuNhapSauThue() {
    ...
    if (thuNhap >= 1000) {
        ...
        if (nguoiPhuThuoc >= 1) {
            soNguoiPhuThuoc = tongThuNhap + mo;
            console.log(soNguoiPhuThuoc)
        }
        // else if (nguoiPhuThuoc >= 2) {
        //     soNguoiPhuThuoc = tongThuNhap * 
        // }
        document.getElementById("thueThuNhap").innerHTML = "Họ tên : " + hoTenKH + "," + " Thuế : " + (new Intl.NumberFormat('vn-VN', { style: 'currency', currency: 'VND' }).format(tongThuNhap)) + " Số tiền người phụ thuộc " + soNguoiPhuThuoc;
    }
};
/* Bài 4: Tính tiền cáp:
